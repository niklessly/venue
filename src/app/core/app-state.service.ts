...
export class AppStateService {
    ...
    readonly filteredRooms = computed(() => {
        ...
        const base = this.rooms().filter((room) => {
            ...
            const fitsEquipment = !query || room.equipment.some((item) => item.toLowerCase().includes(query));
    ...
    readonly statistics = computed<AppStatistics>(() => {
        ...
        const topRoom = [...roomUsage].sort((left, right) => right.count - left.count)[0]?.room.name ?? 'room 1';
        const freeRoom = this.rooms().find((room) => room.status === 'free')?.name ?? this.rooms()[0]?.name ?? 'room';
        const utilization = this.rooms().length ? Math.round((activeBookings.length / this.rooms().length) * 25) : 0;
